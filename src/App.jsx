...
export default function App() {
  ...
  const setCachedToken = (addr, data) => {
    try { tokenCache.current.set(getAddress(addr), { data, timestamp: Date.now() }); }
    catch {}
  };
  ...
  const fetchToken = async () => {
    ...
    try {
      ...
      let checksummedAddr;
      try { checksummedAddr = getAddress(addr); }
      catch { setMessage("Invalid address format."); return; }
      ...
      const [metadataResult, priceResult, dexResult] = await Promise.allSettled([
        ...
        fetchWithRetry(() =>
          axios.get(
            ...
            { params: { chain: BASE_CHAIN }, headers: { "X-API-Key": MORALIS_API_KEY }, timeout: 10000 }
          )
        ),
        fetchWithRetry(() =>
          axios.get(`https://api.dexscreener.com/latest/dex/tokens/${checksummedAddr}`, { timeout: 10000 })
        ),
      ]);

      if (metadataResult.status !== "fulfilled") throw new Error("Failed to fetch token metadata");
      const metadata = metadataResult.value?.data?.[0];
      if (!metadata) throw new Error("Token not found on Base network");

      let price = 0;
      if (priceResult.status === "fulfilled") price = Number(priceResult.value?.data?.usdPrice) || 0;

      let dexData = null, bestPair = null, liquidityUsd = 0;
      ...
      if (dexData?.pairs && Array.isArray(dexData.pairs)) {
        const basePairs = dexData.pairs.filter((p) => p?.chainId === "base" && p?.liquidity?.usd);
        if (basePairs.length > 0) {
          ...
          if (price === 0 && bestPair?.priceUsd) price = Number(bestPair.priceUsd);
          priceChange = {
      ...
      const circulatingSupply = convertSupply(metadata.circulating_supply || metadata.total_supply || "0", decimals);
            ...
            <div className="flex items-center gap-6 sm:gap-8 lg:gap-10">
              ...
              <nav className="hidden lg:flex items-center">
                {[
                  ...
                  >
                    {active && <span className="absolute bottom-0 left-3 right-3 h-px bg-gradient-to-r from-transparent via-cyan-400 to-transparent" />}
                    {label}
                  </a>
                ))}
              </nav>
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {isConnected && !isBase && (
                <div className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-red-500/[0.08] border border-red-500/25">
                  ...
                  <span className="font-mono text-[10px] text-red-400 tracking-wider font-semibold">{chain?.name ?? "Wrong Network"}</span>
                </div>
              )}

              {isConnected && shortAddress && (
                <div className={`hidden sm:flex items-center gap-2 px-3 py-1.5 rounded-lg border ${
                  isBase ? "bg-cyan-400/[0.05] border-cyan-400/[0.18]" : "bg-red-500/[0.05] border-red-500/20"
                }`}>
      ...
      <main className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-10 py-6 sm:py-8 lg:py-14">
        ...
        <div className="flex flex-col lg:flex-row items-start gap-6 sm:gap-5 lg:gap-7">
          ...
          <div className="flex flex-col gap-5 w-full lg:flex-1 min-w-0">
            <div className="relative rounded-2xl overflow-hidden border border-white/[0.06] bg-white/[0.02] backdrop-blur-xl">
              ...
              <div className="p-6 sm:p-5 md:p-6 lg:p-7">
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      <h2 className="font-display text-lg sm:text-base font-bold text-white tracking-tight truncate">Token Explorer</h2>
                      <p className="font-mono text-[11px] sm:text-[10px] text-slate-500 tracking-wider uppercase mt-0.5">Base Network · ERC-20</p>
                    </div>
                  </div>
                ...
                >
                  {!loading && inputAddress && <div className="absolute inset-0 bg-gradient-to-b from-white/[0.08] to-transparent pointer-events-none" />}
                  {loading ? (
                    <>
            ...
            {!tokenData && !loading && (
              <div className="flex flex-col items-center justify-center px-6 py-14 sm:py-16 lg:py-20 rounded-2xl text-center bg-white/[0.01] border border-dashed border-white/[0.06]">
                ...
                <p className="font-mono text-xs sm:text-[11px] text-slate-700 leading-relaxed max-w-[220px]">Enter a contract address above to begin</p>
              </div>
            )}
          </div>
          ...
          <div className="w-full lg:w-[380px] xl:w-[400px] shrink-0 lg:sticky lg:top-[84px]">
            <Swap tokenAddress={fetchedTokenAddress} tokenData={tokenData} ethPrice={ethPrice} />
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 mt-12 sm:mt-14 lg:mt-20">
          {[
            { iconPath: "M13 10V3L4 14h7v7l9-11h-7z",        iconColor: "text-cyan-400",   iconBg: "bg-cyan-400/[0.06] border-cyan-400/[0.12]",   shimmer: "via-cyan-400/25",   hover: "hover:border-cyan-400/[0.15]",   label: "Lightning Fast",    desc: "Execute swaps on Base L2 instantly — near-zero latency, minimal gas." },
            { iconPath: "M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z", iconColor: "text-blue-400", iconBg: "bg-blue-400/[0.06] border-blue-400/[0.12]", shimmer: "via-blue-400/25", hover: "hover:border-blue-400/[0.15]", label: "Non-Custodial", desc: "Your keys, your tokens. We never hold your assets — ever." },
            { iconPath: "M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z", iconColor: "text-violet-400", iconBg: "bg-violet-400/[0.06] border-violet-400/[0.12]", shimmer: "via-violet-400/25", hover: "hover:border-violet-400/[0.15]", label: "Live Analytics", desc: "Real-time prices, liquidity depth, and 24h metrics from DexScreener." },
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 sm:gap-6">
            ...
            {isConnected && shortAddress && (
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs sm:text-[10px] text-emerald-500 tracking-wider tabular-nums">{shortAddress}</span>
              </div>
            )}
          </div>
        </div>
